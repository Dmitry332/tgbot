...
def start(message):
    ...
    markup.add(types.InlineKeyboardButton('Зарегестрироваться', callback_data='start'))
    bot.reply_to(message ,'Вас приветствует бот Random-coffee, зарегестрируйтесь, чтобы получить свою пару', reply_markup=markup)
...
def callback_message(callback):
    if callback.data == 'start':
        ...
        cur.close()
        conn.close()
        bot.send_message(callback.message.chat.id, 'Для регестрации введите ваше имя: ')
...
def user_surname(message):
    ...
    bot.reply_to(message, 'Пользователь зарегистрирован', reply_markup=markup)
